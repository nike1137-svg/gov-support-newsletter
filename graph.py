"""1인 창업 지원 뉴스 브리핑 에이전트 — 1인 창조기업 운영자를 위한 매일 아침 LangGraph 워크플로
...
# 14:57 '중소기업 규모가 요구될 가능성이 높아' 로 대상아님 판정이 통과했다 — '가능성이 있' 만 잡고 있었다
GUESS = re.compile(r"유추|추정|것으로\s*보|판단됨|가능성|불분명|보인다|듯하")
...
def build_messages(today, items, below_min):
    head = f"🗂 <b>{today} · 1인 창업 지원 뉴스 브리핑</b>"
    if not items:
